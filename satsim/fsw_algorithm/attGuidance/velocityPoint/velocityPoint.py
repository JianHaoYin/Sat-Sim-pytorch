...
def rv2elem(mu: torch.Tensor, r_vector: torch.Tensor, v_vector: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    n1_hat = torch.tensor([1, 0, 0], device=r_vector.device, dtype=r_vector.dtype).expand(*batch_shape, 3)
    n3_hat = torch.tensor([0, 0, 1], device=r_vector.device, dtype=r_vector.dtype).expand(*batch_shape, 3)
    ...
    if alpha > eps:
        ...
        r_apoap = torch.zeros([0], device=r_vector.device, dtype=r_vector.dtype)
    ...
    omega = torch.atan2((ih_hat * v_3).sum(dim=-1), (in_hat * ie_hat).sum(dim=-1))
