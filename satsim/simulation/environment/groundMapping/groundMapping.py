__all__ = ["GroundMapping", "GroundMappingStateDict", "GroundStateDict","AccessDict"]
...
class GroundMapping(Module[GroundMappingStateDict]):
    ...
    def __init__(
        ...
        ) if maximum_Range is None else maximum_Range
        ...
        ) if camera_Pos_B is None else camera_Pos_B

        #
        nHat_B = torch.zeros(
            3, dtype=torch.float32
        ) if nHat_B is None else nHat_B
    ...
    def forward(
        ...
        **kwargs,
        ...
        for i in range(len(mapping_Points)):
            #computeAccess
            r_LP_N = torch.matmul(dcm_PN.transpose(-2, -1), mapping_Points[i].unsqueeze(-1)).squeeze(-1)
            ...
            #azimuth shape:[...,1]
            azimuth = torch.atan2(
                (r_BL_L[..., 1] / (torch.sqrt(r_BL_L[..., 0]**2 + r_BL_L[..., 1]**2 + 1e-8))).unsqueeze(-1),
                (-r_BL_L[..., 0] / (torch.sqrt(r_BL_L[..., 0]**2 + r_BL_L[... ,1]**2 + 1e-8))).unsqueeze(-1)
            )
...
def compute_dcm_LP_from_r_LP_N(
    r_LP_N: torch.Tensor,
    dcm_inertial_to_PlanetFix: torch.Tensor
) -> torch.Tensor:
    """
    ...
    # 将地面站位置从惯性系转换到地固系
    r_LP_P = torch.matmul(dcm_inertial_to_PlanetFix, r_LP_N.unsqueeze(-1)).squeeze(-1)
    ...
    dcm_LP = torch.stack([
        ...
        torch.stack([-sinLon,   cosLon,     torch.tensor(0.0, dtype=lat.dtype)],dim=-1),
